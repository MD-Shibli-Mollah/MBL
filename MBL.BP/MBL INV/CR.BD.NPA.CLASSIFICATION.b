* @ValidationCode : MjoxNTQ5NDY5NTY1OkNwMTI1MjoxNjA0MzA3OTc2MzgzOnVzZXI6LTE6LTE6MDowOmZhbHNlOk4vQTpERVZfMjAxNzEwLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 02 Nov 2020 15:06:16
...
*******************************INIT*******************
    ...
    Y.MAINTAIN.MANUAL = ''
***********************END TEST**************************
    IF Y.QJ.MARK NE '' AND Y.MAINTAIN.MANUAL NE '' THEN
    ...
    END
    WriteData = ''
    WriteData = 'Y.AA.ID=':Y.ARR.ID:'*':'Y.BD.NPA.GROUP=':Y.BD.NPA.GROUP:'*':'Y.PRODUCT.LINE=':Y.PRODUCT.LINE:'*':'Y.PRODUCT.GROUP=':Y.PRODUCT.GROUP:'*':'Y.COM=':Y.COM:'*':'Y.MATURITY.DATE=':Y.MATURITY.DATE:'*':'Y.NO.OF.DUE.DAYS=':Y.NO.OF.DUE.DAYS:'*':'Y.AC.MATE.DATE=':Y.AC.MATE.DATE:'*':'Y.MATURITY.DATE=':Y.MATURITY.DATE:'*':'Y.QJ.MARK=':Y.QJ.MARK:'*':'Y.MAINTAIN.MANUAL=':Y.MAINTAIN.MANUAL
    FileName = 'NPA.csv'
    FilePath = 'MBL.DATA'
...
GET.TERM:
*--------
    Y.POSTFIX = RIGHT(Y.TERM,1)
    IF Y.POSTFIX EQ 'Y' THEN
        Y.TERM = Y.TERM[1,LEN(Y.TERM)-1]
    END
    IF Y.POSTFIX EQ 'M' THEN
        Y.TERM = (Y.TERM[1,LEN(Y.TERM)-1])/12
    END
    IF Y.POSTFIX EQ 'D' THEN
        Y.START.DATE = Y.MATURITY.DATE
        Y.DAY = '-':Y.TERM[1,LEN(Y.TERM)-1]:'C'
        EB.API.Cdt('', Y.START.DATE, Y.DAY)
        EB.Utility.NoOfMonths(Y.START.DATE, Y.MATURITY.DATE, Y.MONTH)
        Y.TERM = Y.MONTH/12
    END
RETURN
...
*---------------
    ...
    Y.NO.OF.DUE.DAYS=''
    IF  (Y.BD.NPA.GROUP EQ '10' OR Y.BD.NPA.GROUP EQ '50') AND Y.PRODUCT.LINE EQ 'ACCOUNTS' THEN
        GOSUB CON.AC.GET.UNPAID.DAYS
    END
    IF (Y.BD.NPA.GROUP EQ '30') AND Y.PRODUCT.LINE EQ 'LENDING' THEN
        GOSUB TERM.GET.UNPAID.DAYS
    END
    IF Y.BD.NPA.GROUP EQ '40' OR Y.BD.NPA.GROUP EQ '20' THEN
        GOSUB SHORT.TERM.GET.UNPAID.DAYS
    END
    IF (Y.BD.NPA.GROUP EQ '60' OR Y.BD.NPA.GROUP EQ '70') AND Y.PRODUCT.LINE EQ 'LENDING' THEN
        GOSUB DEMAND.GET.UNPAID.DAYS
    END
RETURN
...
*--------------
    ...
    FOR J = 1 TO Y.TOTAL.SLAB
        ...
        Y.DECISION = FIELD(R.NPA.PARAM<BD.NPA.DECISION>,VM,J)
        IF Y.DECISION EQ 'NR' THEN
            Y.OVERDUE.INSTL.TO = '9999999999'
        END
        ELSE
